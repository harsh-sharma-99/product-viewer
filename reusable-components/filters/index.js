...
const Filters = ({ data }) => {
  ...
  const _onSelectedProducts = (e) => {
    ...
    typeof e === "undefined"
      ? setSelectedProductFlag(false)
      : setSelectedProductFlag(true);
    if (e) {
  ...
  const _onSelectedState = (e) => {
    if (!selectedProductFlag) {
      ...
      const filteredData = data.filter(
        (prod) => prod.product_name === selProductVal
      );
      const uniqueStates = [
        ...new Set(filteredData.map((item) => item.address.state)),
      ];
      setStateDropdown(uniqueStates);
    }
    typeof e === "undefined"
  ...
  const _onSelectedCity = () => {
    ...
    if (selectedProductFlag && !selectedStateFlag) {
      ...
      const uniqueCities = [
        ...new Set(filteredData.map((item) => item.address.city)),
      ];
      setCityDropdown(uniqueCities);
    }
    if (!selectedProductFlag && selectedStateFlag) {
      const filteredData = data.filter(
        (prod) => prod.address.state === selStateVal
      );
      const uniqueCities = [
        ...new Set(filteredData.map((item) => item.address.city)),
      ];
    ...
    if (selectedProductFlag && selectedStateFlag) {
      const filteredData = data.filter(
        (prod) =>
          prod.address.state === selStateVal &&
          prod.product_name === selProductVal
      );
      const uniqueCities = [
        ...new Set(filteredData.map((item) => item.address.city)),
      ];
